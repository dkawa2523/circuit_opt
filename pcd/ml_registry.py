...
def register(kind: str, name: str) -> Callable[[Callable[..., Any]], Callable[..., Any]]:
    if kind not in REGISTRY:
        raise KeyError(f"unknown ML method kind: {kind}. expected one of {list(REGISTRY)}")
    ...
    def deco(func: Callable[..., Any]) -> Callable[..., Any]:
        ...
        if existing is not None and existing is not func:
            ...
            warnings.warn(
                f"ML method conflict for {kind}:{name}; keeping {conflict['existing']}",
                RuntimeWarning,
                stacklevel=2,
            )
            return func
...
def _load_builtins() -> None:
    ...
    if not _BUILTINS_LOADED:
        importlib.import_module("pcd.ml_methods")
